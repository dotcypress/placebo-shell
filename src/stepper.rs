...
impl<A: OutputPin, B: OutputPin, C: OutputPin, D: OutputPin> Stepper<A, B, C, D> {
    ...
    pub fn turn(&mut self) {
        ...
        drive_pin(&mut self.pins.0, STEPS[self.step] & 0b1000 == 0b1000);
        drive_pin(&mut self.pins.1, STEPS[self.step] & 0b0100 == 0b0100);
        drive_pin(&mut self.pins.2, STEPS[self.step] & 0b0010 == 0b0010);
        drive_pin(&mut self.pins.3, STEPS[self.step] & 0b0001 == 0b0001);
    }
}
